...
public class Pawn extends Piece{
	...
	public int calculateValue() {
		...
		Coordinate tempCoord = getCoordinate();
		try {
			//If pawn is a passed pawn, it has more value
			switch(getTeam()) {
			case WHITE :

				for(int i = getCoordinate().getRank(); i <= 8; i++) {
					tempCoord = tempCoord.up();
					if(Board.getTile(tempCoord).isOccupiedByBlack()) {
						passedPawn = false;
						break;
					}
				}
			...
			case BLACK :
				...
				for(int i = getCoordinate().getRank(); i >= 1; i--) {
					tempCoord = tempCoord.down();
					if(Board.getTile(tempCoord).isOccupiedByWhite()) {
						passedPawn = false;
						break;
					}
				}
		...
		if(passedPawn)value += 5;
		
		//if pawn is close to center
