...
public class Knight extends Piece{
	...
	public int calculateValue() {
		int value = getValue();
		int numberOfPiecesAttacked = 0;

		//If Knight forks two pieces and isn't attacked(+10), or is controlled(+3)
		Coordinate temp;
		...
		case WHITE :
			for(int j = 1; j <= 8; j++) {
				...
				&& Board.getTile(temp).isOccupiedByBlack()){
					if(Board.getTile(temp).getPiece().getClass()!=Pawn.class){
						numberOfPiecesAttacked++;
					}	
				}
			}
			
			if(numberOfPiecesAttacked>1) {
				if(!Board.black.controls(getCoordinate())) {
					value+=10;
				}else if(Board.white.controls(getCoordinate())) {
					value+=3;
				}
			}
		...
		case BLACK :
			for(int j = 1; j <= 8; j++) {
				...
				if(numberOfPiecesAttacked>1) {
					if(!Board.white.controls(getCoordinate())) {
						value+=10;
					}else if(Board.black.controls(getCoordinate())) {
						value+=3;
					}
				}
			}
